    ...
    if (existing.length > 0 && existing[0].values.length > 0) {
        ...
        db.run(`
            UPDATE transcripts SET
                ...
                raw_json = ?, clean_text = ?, language = ?, is_auto_generated = ?,
                word_count = ?, captured_at = datetime('now')
            WHERE video_id = ?
        `, [title, channelName, videoUrl, durationSeconds, rawJson, cleanText,
            language, isAutoGenerated ? 1 : 0, wordCount, videoId]);
    } else {
        // Insert
        db.run(`
            INSERT INTO transcripts
            (video_id, title, channel_name, video_url, duration_seconds,
             raw_json, clean_text, language, is_auto_generated, word_count)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [videoId, title, channelName, videoUrl, durationSeconds, rawJson, cleanText,
