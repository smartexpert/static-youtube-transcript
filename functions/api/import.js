    ...
    try {
        ...
        for (const t of transcripts) {
            try {
                ...
                const channelName = t.channel_name ?? null;
                const videoUrl = t.video_url ?? null;
                const durationSeconds = t.duration_seconds ?? null;
                const publishDate = t.publish_date ?? null;
                const rawJson = t.raw_json ?? null;
                const cleanText = t.clean_text ?? null;
                const language = t.language ?? null;
                const isAutoGenerated = t.is_auto_generated ? 1 : 0;
                const wordCount = t.word_count ?? null;
                ...
                if (existing) {
                    ...
                    await env.DB.prepare(`
                        ...
                        WHERE video_id = ?
                    `).bind(
                        title, channelName, videoUrl, durationSeconds,
                        publishDate, rawJson, cleanText, language,
                        isAutoGenerated, wordCount, videoId
                    ).run();
                } else {
                    // Insert
                    await env.DB.prepare(`
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    `).bind(
                        videoId, title, channelName, videoUrl, durationSeconds,
                        publishDate, rawJson, cleanText, language,
                        isAutoGenerated, wordCount
                    ).run();
                }
                imported++;
            } catch (err) {
